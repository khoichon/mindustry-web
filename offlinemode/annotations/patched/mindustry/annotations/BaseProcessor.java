...
/**
 * Copy of annotations/src/main/java/mindustry/annotations/BaseProcessor.java
 * for the TeaVM build, with one change (everything else is verbatim upstream):
 * the processor "root directory" (which AssetsProcess and EntityProcess read
 * and WRITE under -- core/assets*, annotations/src/main/resources/revisions,
 * classids.properties) can be set explicitly through the standard annotation
 * processor option -AmindustryRoot=..., instead of only being inferred by
 * climbing seven parents from the compiler output directory. The TeaVM build
 * compiles from a staged source tree whose output path does not sit inside
 * the Mindustry repo layout, and must never write into the pristine
 * reference checkout -- build.gradle stages a writable copy of exactly the
 * subdirectories the processors touch and points this option at it.
 * Swapped in via source-set filtering in annotations/build.gradle.
 */
...
public abstract class BaseProcessor extends AbstractProcessor{
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv){
        ...
        if(rootDirectory == null){
            // TeaVM build: explicit -AmindustryRoot=... takes precedence (see
            // class doc). Everything below the if is the upstream heuristic.
            String override = processingEnv.getOptions().get("mindustryRoot");
            if(override != null && !override.isEmpty()){
                rootDirectory = Fi.get(override);
            }
        }
        if(rootDirectory == null){
            try{
