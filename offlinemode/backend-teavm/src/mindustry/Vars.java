...
public class Vars implements Loadable{
    ...
    public static TeavmSimpleExecutor mainExecutor = Core.executor;
    ...
    public static void init(){
        // TEMP DIAGNOSTIC (TeaVM): step markers to find where init() dies.
        Groups.init();
        ...
        // TeaVM: there is no java.home property in the browser, and Fi's
        // constructor NPEs on a null path -- guard the first branch. javaPath
        // is only ever used to spawn processes, which never happens on the
        // web backend (no Runtime.exec).
        String javaHome = OS.prop("java.home");
        javaPath =
            javaHome != null && new Fi(javaHome).child("bin/java").exists() ? new Fi(javaHome).child("bin/java").absolutePath() :
    ...
    public static void loadSettings(){
        ...
        try{
            ...
            // WEB DELTA: TeaVM's Currency.getInstance(Locale) takes a
            // garbage path for a LANGUAGE-ONLY default locale (empty
            // country) and throws "Currency not found: CYP" from every
            // locale-derived formatter, freezing boot (user-reported with a
            // language-only saved locale; a default locale that carries a
            // country -- e.g. the navigator-derived en_US -- is fine).
            // Always give the JAVA default locale a country; the bundle
            // lookup below keeps using the full `locale` unchanged.
            Locale def = locale;
            if(def.getCountry() == null || def.getCountry().isEmpty()){
                def = new Locale(def.getLanguage(), "US");
            }
