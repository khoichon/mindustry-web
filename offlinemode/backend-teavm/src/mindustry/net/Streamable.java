...
public class Streamable extends Packet{
    ...
    static class IncrementalStream extends InputStream{
        ...
        // TeaVM swap: upstream uses java.util.concurrent.LinkedBlockingQueue,
        // which TeaVM's classlib lacks entirely. Same contract here: unbounded
        // FIFO, take() parks until an element arrives. wait/notify compile to
        // coroutine parking under TeaVM, so a reader parked in take() lets the
        // writer coroutine make progress on the same physical thread.
        ...
        /** see the queue field: minimal LinkedBlockingQueue for TeaVM. */
        static class ByteQueue{
            private final ArrayDeque<byte[]> deque = new ArrayDeque<>();
            ...
            byte[] take() throws InterruptedException{
                synchronized(deque){
                    while(deque.isEmpty()) deque.wait();
