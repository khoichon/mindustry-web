...
public class SoundControl{
    ...
    static class AudioThread extends Thread{
        ...
        // TeaVM swap: classlib has no LinkedBlockingQueue; only add() and poll()
        // are ever called on this queue, which ArrayDeque provides natively
        // (both coroutines run on one physical thread, so no locking needed).
        ArrayDeque<AmbientSource> inputSources = new ArrayDeque<>();
            ...
            try{
                while(running){
                    ...
                    if(elapsed < targetNanos){
                        ...
                        // TeaVM: no Thread.sleep(long, int); millisecond precision only
                        Thread.sleep(remaining / Time.nanosPerMilli);
                    }
                }
