...
/**
 * Copy of core/src/mindustry/async/PhysicsProcess.java for the TeaVM build,
 * with the async plumbing removed: upstream submits each layer's update to
 * Vars.mainExecutor and blocks on the futures at the next begin(); under
 * TeaVM there is one physical thread, so the updates run inline instead.
...
public class PhysicsProcess implements AsyncProcess{
    ...
    public void begin(){
        ...
        //update every world inline -- no parallelism exists on this platform
        long begin = Time.nanos();
        for(int i = 0; i < layers; i++){
            physics[i].update();
        }
        maxPhysicsTime = Time.timeSinceNanos(begin);
    }
