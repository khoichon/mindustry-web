...
public class Mods implements Loadable{
    ...
    public void loadAsync(){
        ...
        var tasks = new Seq<TeavmFuture<Runnable>>();
    ...
    private void packSprites(MultiPacker packer, Seq<Fi> sprites, LoadedMod mod, boolean prefix, Seq<TeavmFuture<Runnable>> tasks, ObjectFloatMap<String> textureResize){
    ...
    void waitForMain(Runnable run){
        // TeaVM swap: upstream blocks on a CountDownLatch until the posted
        // runnable executes on the main loop. There is exactly one physical
        // thread here, so blocking could only ever deadlock (the main loop
        // could not advance to run the posted task) -- and running inline is
        // equivalent anyway, since the "background" mod-packing coroutine and
        // the main loop share the same thread and the same GL context.
        run.run();
    }
